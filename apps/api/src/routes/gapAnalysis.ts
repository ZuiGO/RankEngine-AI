...
router.post('/:id/keyword-gap', async (req: Request, res: Response) => {
  try {
...
router.post('/:id/backlink-gap', async (req: Request, res: Response) => {
  try {
    const project = await resolveProject(req, res);
    if (!project) return;
    ...
    const results = await Promise.all(
      domains.map((domain) =>
        limit(async () => {
          try {
            const domains_list = await getReferringDomains(domain);
