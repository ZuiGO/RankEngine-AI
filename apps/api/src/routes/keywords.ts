...
// GET /api/projects/:id/keywords - List all tracked keywords with current positions
...
// GET /api/projects/:id/keywords/:keywordId/history - Get 30 days history
router.get('/:id/keywords/:keywordId/history', async (req: Request, res: Response) => {
  try {
...
router.get('/:id/suggested-keywords', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    if (!isValidObjectId(id)) {
      return res.status(400).json({ error: 'Invalid project ID format' });
    ...
    const active = (project.suggestedKeywords || []).filter((sk) => !sk.dismissed);
    return res.json(active);
  } catch (error) {
    console.error('List suggested keywords error:', error);
    return res.status(500).json({ error: 'Internal server error' });
  }
});

// POST /api/projects/:id/suggested-keywords/:index/dismiss
router.post('/:id/suggested-keywords/:index/dismiss', async (req: Request, res: Response) => {
  try {
